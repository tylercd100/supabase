    ...
    <Tabs
      ...
      onValueChange={(value) => {
        if (value === 'https') {
          ...
          setFieldValue(uriId, httpsValues[uriId])
          setFieldValue(enabledId, httpsValues[enabledId])
        } else {
          setHttpsValues({ [uriId]: values[uriId], [enabledId]: values[enabledId] })
          setFieldValue(uriId, postgresValues[uriId])
          setFieldValue(enabledId, postgresValues[enabledId])
        }
      ...
      <TabsContent value="https">
        <div className="flex flex-col gap-4">
          <Input
            ...
            descriptionText="Supabase Auth will send a POST request to this endpoint each time the hook is run."
